...
class Res101Encoder(nn.Module):
    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='resnet101'):
        ...
        if pretrained_weights == 'deeplabv3':
            ...
            self.pretrained_weights = torch.load("/home/cs4007/data/zyz/BAT/checkpoints/resnet101-63fe2227.pth",
                                                 map_location='cpu')
        else:
    ...
    def forward(self, x):
        ...
        x = self.backbone["layer2"](x)  # size of x is [2, 512, 64, 64]
        ...
        t = self.backbone["avgpool"](x)  # (2, 2048, 1, 1)
        t = torch.flatten(t, 1)  # (2, 2048)
        t = self.backbone["fc"](t)  # (2, 1000)
        t = self.reduce1d(t)  # (2, 1)
