...
from models.positional_encoding import SinePositionalEncoding
...
class CyCTransformer(nn.Module):
    ...
    def forward(self, x, qry_masks, s_x, supp_mask, supp_protos,init_mask,thresh):
        # print(f"x:{x.shape}")       1 X 512 X 64 X 64
        # print(f"qry_masks:{qry_masks.shape}")       1 X 256X 256
        # print(f"s_x:{s_x.shape}")       1 X 512 X 64 X 64
        # print(f"supp_mask:{supp_mask.shape}")      1 X 1 X 256 X 256
        # print(f"init_mask:{init_mask.shape}")       1 X 64 X 64
        ...
        for n in range(supp_protos.shape[0]):
            ln_id = 0
            ffn_id = 0
            # res_id=0
            #
            ...
            k = s_x
            v = k.clone()
            supp_proto=supp_protos[[n],:,:,:].squeeze(-1).permute(0, 2, 1)

            #print(f"supp_proto:{supp_proto.shape},k:{k.shape},supp_mask_flatten:{supp_mask_flatten.shape},supp_valid_mask:{supp_valid_mask.shape}")
            #supp_proto:torch.Size([1, 1, 512]),k:torch.Size([1, 4096, 512]),supp_mask_flatten:torch.Size([1, 4096]),supp_valid_mask:torch.Size([1, 4096])
            prototype = supp_proto+self.cross_layers(supp_proto, k, v, supp_valid_mask, supp_mask_flatten)
            prototype = self.layer_norms[0](prototype)

            prototype = self.ffns[ffn_id](prototype)
            ffn_id+=1
            prototype = self.layer_norms[1](prototype)
            #qry_outputs_mask, _ = self.forward_prediction_heads(prototype, x_flatten,thresh)
            ...
            #tmp_q = rearrange(torch.cat((q, tmp_prototype), dim=-1), 'b (h w) c -> b c h w', h=64, w=64)
            # q = self.update_q[res_id](tmp_q)
            # q = q + self.res_q[res_id](q)
            # res_id+=1
            #q = rearrange(q, 'b c h w -> b (h w) c')
            #q=torch.cat((q, tmp_prototype), dim=-1)
            ...
            for l_id in range(self.num_layers):
                ...
                qry_prototype = qry_prototype+self.decoder_cross_attention_layers[l_id](
                    qry_prototype, q, qry_attn_mask)
                qry_prototype_list.append(qry_prototype)
            #q=rearrange(q, 'b (h w) c -> b c h w', h=64, w=64)

            #qry_outputs_mask, qry_attn_mask = self.forward_prediction_heads(prototype, x_flatten,thresh)
        ...
        qry_prototype=torch.cat(qry_prototype_list,dim=1)
        #out = qry_outputs_mask_list[-1].clone()
        return qry_prototype.squeeze(0)

    def forward_prediction_heads(self, fts, prototype, thresh):
        """
        Calculate the distance between features and prototypes

        Args:
            fts: input features
                expect shape: 1 x (H * W) x C
            prototype: prototype of one semantic class
                expect shape: 1 x 1 x C
        """
        fts=fts.permute(0, 2, 1)
        prototype=prototype.permute(0, 2, 1)

        sim = -F.cosine_similarity(fts, prototype, dim=1) * self.scaler
        #print(f"sim:{sim.shape}")  1*4096
        pred = 1.0 - torch.sigmoid(0.5 * (sim - thresh))

        pred = torch.stack((1.0 - pred, pred), dim=1)  # 1*2*(64*64)
        attn_mask = pred.max(1)[1].detach()
        pred=rearrange(pred, 'b c (h w) -> b c h w', w=64, h=64)
