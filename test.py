...
from models.ASGNet import FewShotSeg
    ...
    for label_val, label_name in labels.items():
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                #print(f"query_image:{query_image[0].shape}")   query_image:torch.Size([24, 3, 256, 256])
                #print(f"query_label:{query_label.shape}")  query_label:torch.Size([1, 24, 256, 256])
                ...
                for sub_chunck in range(_config['n_part']):  # n_part = 3
                    ...
                    qry_mask_s=query_label[0,idx_[sub_chunck]:idx_[sub_chunck + 1]]
                    # print(f"query_image_s:{query_image_s.shape}")
                    # print(f"qry_mask_s:{qry_mask_s.shape}")
                    query_pred_s = []
                    for i in range(query_image_s.shape[0]):
                        _pred_s, _, _,_ = model([support_image_s], [support_fg_mask_s], [query_image_s[[i]]],qry_mask=qry_mask_s[[i]].cuda(),train=False)  # 1 x 2 x H x W
